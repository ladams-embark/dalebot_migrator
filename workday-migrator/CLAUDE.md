...
The scaffold exists but all modules are empty. Build order:
...
## Testing approach
- `test_auth.py` — calls `verify_connection()` against the source tenant only (no writes). Requires a live `.env`.
- `test_discovery.py` — calls `get_all_calculated_fields()` and asserts non-empty list. Live source tenant.
- `test_ordering.py` — pure unit tests with fixture data, no tenant required.
- `test_writer.py` — always runs with `dry_run=True`. Asserts correct log output, zero SOAP calls to destination.
- `test_validation.py` — reads from destination only (no writes).
